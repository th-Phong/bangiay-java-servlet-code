 ...
 */
@WebServlet({"/DetailProductServlet","/detailproduct.html"})
public class DetailProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		List<Product> chitietsanpham = ProductBL.DocChiTiet(idpro);
		request.setAttribute("chitietsanpham", chitietsanpham);
		
		// request.getRequestDispatcher("detail.jsp").forward(request, response);

		List<Product> dsp_new2 = ProductBL.LaySanPhamMoi();
		request.setAttribute("dsp_new2", dsp_new2);
